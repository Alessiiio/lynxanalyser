...
- **Akte L5-Treffer mit Netz**: Nach Suchweite 5 siehst du das Beziehungsnetz. Aktive Organe der Fraud-Firma und umliegende Firmen kannst du gezielt markieren — Unbeteiligte sind nicht mehr vorausgewählt.
...
- **Design System dokumentiert**: `docs/DESIGN_SYSTEM.md` als Referenz für Buttons (`.btn`), Badges und Cards (additiv; bestehende Lynx-Klassen bleiben gültig).
- **Admin-Panel Optik**: Hub behält Tabs/Exporte/Audit/Betrieb, nutzt wieder die bewährten dunklen Panels und `btn-nav` / `btn-check` (kein helles Design-System-`.btn` auf der Admin-Seite).
